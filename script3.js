...
document.getElementById("generateFixture").addEventListener("click", () => {
  matches = generateBalancedFixture(teams);
  saveState();
  renderFixture();
});

function generateBalancedFixture(teams) {
  const matches = [];
  const fixture = [];
  ...
  // Generar fixture usando el algoritmo de Round-Robin
  let rotatingTeams = teams.slice(1); // Dejar el primer equipo fijo
  ...
  // Aplanar el fixture para que sea una lista de partidos
  fixture.forEach((round) => matches.push(...round));
...
function renderFixture() {
  ...
  matches.forEach((match, index) => {
    ...
    matchDiv.innerHTML = `
      ...
      <button onclick="startTimer(${index}, 10)">Start Timer</button>
      <div id="timer-${index}" class="timer-display">10</div>
      <hr>
    `;
...
function startTimer(index, timeInSeconds) {
